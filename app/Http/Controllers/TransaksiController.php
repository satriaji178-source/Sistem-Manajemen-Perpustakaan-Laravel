...
class TransaksiController extends Controller
{
    ...
    public function kembalikan(string $id)
    {
        try {
            ...
            return redirect()->route('transaksi.show', $id)
                             ->with('success', 'Buku berhasil dikembalikan!'. ($transaksi->denda > 0 ? 'Total denda keterlambatan: Rp ' . number_format($transaksi->denda, 0, ',', '.') : 'Buku dikembalikan tepat waktu.'));
    ...
    public function laporan(Request $request)
    {
        ...
        // 1. Filter Range Tanggal (dari - sampai) berdasarkan tanggal_pinjam
    ...
    {
        // Logika filter yang sama persis untuk PDF
        $query = Transaksi::with(['anggota', 'buku']);
        ...
        $pdf = Pdf::loadView('transaksi.laporan_pdf', compact('transaksis', 'totalTransaksi', 'totalDenda', 'request'))
                ->setPaper('a4', 'landscape'); // Menggunakan landscape agar tabel muat
